...
inline Vector3 DirectX::SimpleMath::Vector3::MyMul(const Vector3& v1, float s)
{  
    XMFLOAT3 R{};
    R.x = v1.x * s;
    R.y = v1.y * s;
...
inline Vector3 DirectX::SimpleMath::Vector3::MyAdd(const Vector3& v1, const Vector3& v2)
{
    XMFLOAT3 R{};
    R.x = v1.x + v2.x;
    R.y = v1.y + v2.y;
...
inline Vector3 DirectX::SimpleMath::Vector3::MyDiv(const Vector3& v1, float s)
{
    XMFLOAT3 R{};
    R.x = v1.x / s;
    R.y = v1.y / s;
...
inline float DirectX::SimpleMath::Vector3::Dot(const Vector3& v1, const Vector3& v2) noexcept{

    XMVECTOR inv1, inv2, outv;
    XMFLOAT3 outfloat;
    inv1 = XMLoadFloat3(&v1);
    inv2 = XMLoadFloat3(&v2);
    outv = XMVector3Dot(inv1, inv2);
    XMStoreFloat3(&outfloat, outv);
    return outfloat.x;
}

inline Vector3 DirectX::SimpleMath::Vector3::Cross(const Vector3& v1, const Vector3& v2) noexcept
{
    XMVECTOR inv1, inv2, outv;
    XMFLOAT3 outvec;
    inv1 = XMLoadFloat3(&v1);
...
inline Vector3 DirectX::SimpleMath::Vector3::normalize(const Vector3& v1) noexcept{

    XMVECTOR inv, outv;
    XMFLOAT3 outvec;
    inv = XMLoadFloat3(&v1);
    outv = XMVector3Normalize(inv);
    XMStoreFloat3(&outvec, outv);
    return outvec;
}
...
inline void DirectX::SimpleMath::Vector3::Slerp(const Vector3& v1, const Vector3& v2, float t, Vector3& result) noexcept{
}


inline Vector3 DirectX::SimpleMath::Vector3::Slerp(const Vector3& v1, const Vector3& v2, float t) noexcept{

    Vector3 s = XMVector3Normalize(v1);
    Vector3 e = XMVector3Normalize(v2);
    ...
    //Vector3 out = (s * Ps + e * Pe) / SinTh;
    XMFLOAT3 out = MyDiv(MyAdd(MyMul(s, Ps), MyMul(e, Pe)), SinTh);
    Vector3 normalout = out;
...
inline Matrix DirectX::SimpleMath::Matrix::LeftHand() const noexcept
{
    Vector3 pos = this->Translation();
    Quaternion qt = Quaternion::CreateFromRotationMatrix(*this);

    pos.x *= -1.0f;
    qt.x *= -1.0f;
    qt.w *= -1.0f;

    Matrix mat = Matrix::CreateFromQuaternion(qt);
    mat._41 = pos.x;  mat._42 = pos.y;  mat._43 = pos.z;

    return mat;
}

inline void DirectX::SimpleMath::Matrix::LeftHand(Matrix& result) const noexcept
{
    Vector3 pos = this->Translation();
    Quaternion qt = Quaternion::CreateFromRotationMatrix(*this);
    ...
    result = Matrix::CreateFromQuaternion(qt);
...
inline Vector3 Matrix::ToEuler() const noexcept
{
    ...
    {
        return Vector3(XMConvertToDegrees(cx), XMConvertToDegrees(atan2f(_31, _33)), XMConvertToDegrees(atan2f(_12, _22)));
    }
    else
    {
        return Vector3(XMConvertToDegrees(cx), XMConvertToDegrees(0.f), XMConvertToDegrees(atan2f(-_21, _11)));
    }
}
...
inline Matrix DirectX::SimpleMath::Matrix::Slerp(const Matrix& M1, const Matrix& M2, float t) noexcept
{
    // 各姿勢ベクトル抽出
    Vector3 Sy = { M1._21,M1._22,M1._23 }, Sz = { M1._31,M1._32,M1._33 };
    Vector3 Ey = { M2._21,M2._22,M2._23 }, Ez = { M2._21,M2._22,M2._23 };
    Vector3 Sp = { M1._41,M1._42,M1._43 }, Ep = { M2._41,M2._42,M2._43 };

    // 中間ベクトル算出
    Vector3 IY, IZ, IP;
    IY = Vector3::Slerp(Sy, Ey, t);
    IZ = Vector3::Slerp(Sz, Ez, t);
    IP = Vector3::Slerp(Sp, Ep, t);

    Vector3 IX;
    IX = Vector3::Cross(IY, IZ);
    IY = Vector3::Cross(IZ, IX);
    IX.Normalize();
    IY.Normalize();
    IZ.Normalize();
